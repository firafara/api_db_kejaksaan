...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['user_id']) && isset($_FILES['laporan_pengaduan_pdf'])  && isset($_FILES['ktp_pdf']) && isset($_POST['laporan_pengaduan']) && isset($_POST['ktp']) && isset($_POST['nama_pelapor']) && isset($_POST['no_hp'])) {
        ...
        $sql = "INSERT INTO tb_pengawasan (user_id, laporan_pengaduan_pdf, ktp_pdf, laporan_pengaduan, ktp, nama_pelapor, no_hp) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($stmt === false) {
            ...
            if ($stmt->execute()) {
                ...
                $response['message'] = "Berhasil menambahkan laporan pengawasan";
            } else {
                $response['isSuccess'] = false;
                $response['message'] = "Gagal menambahkan laporan pengawasan: " . $stmt->error;
            }
            $stmt->close();
        }
    } else {
