...
//                     $sql = "INSERT INTO tb_pilkada (user_id, laporan_pengaduan_pdf, laporan_pengaduan) VALUES (?, ?, ?)";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['user_id']) && isset($_FILES['laporan_pengaduan_pdf'])  && isset($_FILES['ktp_pdf']) && isset($_POST['laporan_pengaduan']) && isset($_POST['ktp']) && isset($_POST['nama_pelapor']) && isset($_POST['no_hp'])) {
        ...
        // Insert into database
        $sql = "INSERT INTO tb_pilkada (user_id, laporan_pengaduan_pdf, ktp_pdf, laporan_pengaduan, ktp, nama_pelapor, no_hp) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $koneksi->prepare($sql);
        if ($stmt === false) {
            $response['isSuccess'] = false;
            $response['message'] = "Failed to prepare statement: " . $koneksi->error;
            ...
            if ($stmt->execute()) {
                ...
                $response['isSuccess'] = true;
                $response['message'] = "Berhasil menambahkan laporan pilkada";
            } else {
                $response['isSuccess'] = false;
                $response['message'] = "Gagal menambahkan laporan pilkada: " . $stmt->error;
            }
            $stmt->close();
        }
    } else {
...
//             $sql = "INSERT INTO tb_pilkada (user_id, laporan_pengaduan_pdf, laporan_pengaduan, status, created_at, updated_at) VALUES ('$user_id', '$laporanPengaduanPdf', '$laporan_pengaduan', '$status', NOW(), NOW())";
